...
public class MainActivity extends AppCompatActivity {
    ...
    private ViewGroup webViewOriginalParent;
    private ViewGroup.LayoutParams webViewOriginalLayoutParams;
    private int webViewOriginalIndex=-1;
    ...
    private TextView participantCount, connectionLabel, mediaLabel, syncStatusLabel, participantSummary, roomWatchingLabel;
    ...
    private final BroadcastReceiver receiver = new BroadcastReceiver() {
        @Override public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                if ("connected".equals(ev)) {
                    ...
                    runOnUiThread(() -> {
                        if (!isFinishing()) {
                            ...
                            } else { if (connectionLabel != null) { connectionLabel.setText("●  Connected"); connectionLabel.setTextColor(Color.rgb(0,255,115)); } sendStateRequest(); }
                            refreshParticipants();
                        }
                    });
                    ...
                    runOnUiThread(() -> { if(connectionLabel!=null) { connectionLabel.setText("●  Reconnecting…"); connectionLabel.setTextColor(MUTED); } if(syncStatusLabel!=null) { syncStatusLabel.setText("Connection interrupted"); syncStatusLabel.setTextColor(MUTED); } });
                    ...
                    applySync(d);
                    runOnUiThread(()->{if(syncStatusLabel!=null) { syncStatusLabel.setText("✓  Synced"); syncStatusLabel.setTextColor(Color.rgb(0,255,115)); }refreshParticipants();});
                ...
                else if ("chat".equals(ev)) { String senderId=d.optString("senderId",""); if(!localPresenceKey.equals(senderId)) { addChat(d.optString("user","Unknown"), d.optString("text",""), d.optString("pfp",""), d.optLong("sentAt",System.currentTimeMillis()), d.optString("messageId",""), senderId); if(!chatOpen){ unreadMessages=Math.min(99,unreadMessages+1); runOnUiThread(MainActivity.this::updateChatBadges); } playChatSound(); } }
    ...
    private LinearLayout sourceCard(String label,String detail,int iconRes,boolean enabled){
        ...
        card.setBackground(round(Color.rgb(8,8,8),LINE,20));
    ...
    private void showConnect(){
        ...
        ImageView pwIcon=iconView(R.drawable.ic_lock,24); pwIcon.setLayoutParams(new LinearLayout.LayoutParams(dp(30),dp(30)));
    ...
    private Intent serviceIntent(String action){
        return new Intent(this,SyncService.class).setAction(action);
    }
    ...
    private void connect(String server,String key,String user,String room,String pass){
        ...
        Intent i=serviceIntent(SyncService.ACTION_CONNECT).putExtra("server",server).putExtra("key",key).putExtra("user",username).putExtra("room",room).putExtra("password",pass).putExtra("pfp",profilePicture);
    ...
    private void showRoom(){
        ...
        roomSessionGeneration++;roomRoot=new FrameLayout(this);roomRoot.setBackground(createRoomBackground());ScrollView sc=scroll(col());roomContent=(LinearLayout)sc.getChildAt(0);roomContent.setPadding(dp(10),dp(10),dp(10),dp(78));FrameLayout.LayoutParams scLp=new FrameLayout.LayoutParams(-1,-1);roomRoot.addView(sc,scLp);screen(roomRoot);
        roomHeader=row();roomHeader.setGravity(Gravity.CENTER_VERTICAL);ImageButton back=new ImageButton(this);back.setImageResource(R.drawable.ic_back);back.setColorFilter(TEXT);back.setBackgroundColor(Color.TRANSPARENT);back.setPadding(0,0,0,0);back.setScaleType(ImageView.ScaleType.CENTER);roomHeader.addView(back,new LinearLayout.LayoutParams(dp(30),dp(36)));TextView h=title(roomName,14);h.setGravity(Gravity.CENTER_VERTICAL);h.setSingleLine(true);roomHeader.addView(h,new LinearLayout.LayoutParams(0,dp(36),1));syncStatusLabel=text("✓  Synced",10,Color.rgb(0,255,115));syncStatusLabel.setTypeface(Typeface.DEFAULT,Typeface.BOLD);syncStatusLabel.setGravity(Gravity.CENTER);roomHeader.addView(syncStatusLabel,new LinearLayout.LayoutParams(dp(58),dp(32)));ImageButton settingsTop=iconOnlyButton(R.drawable.ic_settings,false);settingsTop.setPadding(dp(4),dp(4),dp(4),dp(4));roomHeader.addView(settingsTop,new LinearLayout.LayoutParams(dp(36),dp(36)));add(roomContent,roomHeader,-1,dp(36),0,8);back.setOnClickListener(v->disconnect());settingsTop.setOnClickListener(v->showSettingsDialog());
        LinearLayout statusRow=row();connectionLabel=text("●  Connected",10,Color.rgb(0,255,115));connectionLabel.setTypeface(Typeface.DEFAULT,Typeface.BOLD);connectionLabel.setGravity(Gravity.CENTER_VERTICAL);statusRow.setPadding(dp(2),0,dp(2),0);statusRow.addView(connectionLabel,new LinearLayout.LayoutParams(0,dp(22),1));roomWatchingLabel=text(formatPeopleWatching(Math.max(1,participants.size())),10,Color.rgb(0,255,115));roomWatchingLabel.setTypeface(Typeface.DEFAULT,Typeface.BOLD);roomWatchingLabel.setGravity(Gravity.CENTER_VERTICAL|Gravity.RIGHT);statusRow.addView(roomWatchingLabel,new LinearLayout.LayoutParams(0,dp(22),1));add(roomContent,statusRow,-1,dp(22),0,8);
        roomPeople=col();roomPeople.setPadding(dp(12),dp(10),dp(12),dp(10));roomPeople.setBackground(round(Color.rgb(5,5,5),LINE,14));LinearLayout person=row();person.setGravity(Gravity.CENTER_VERTICAL);ImageView avatar=new ImageView(this);avatar.setImageBitmap(loadPfpBitmap(participantPfps.getOrDefault(localPresenceKey,"asset:0")));avatar.setScaleType(ImageView.ScaleType.CENTER_CROP);avatar.setBackground(round(SURFACE2,Color.TRANSPARENT,20));person.addView(avatar,new LinearLayout.LayoutParams(dp(38),dp(38)));LinearLayout words=col();words.setGravity(Gravity.CENTER_VERTICAL);words.setPadding(dp(8),0,0,0);String localName=username==null||username.trim().isEmpty()?"AriaSyetan":username;TextView localNameView=title(localName,12);localNameView.setSingleLine(true);localNameView.setEllipsize(android.text.TextUtils.TruncateAt.END);localNameView.setIncludeFontPadding(true);add(words,localNameView,-1,dp(30),0,1);add(words,text("●  Online",10,Color.rgb(0,255,115)),-1,dp(24),0,0);person.addView(words,new LinearLayout.LayoutParams(0,dp(54),1));roomPeople.addView(person,new LinearLayout.LayoutParams(-1,dp(54)));add(roomContent,roomPeople,-1,dp(74),0,8);roomPeople.setOnClickListener(v->showParticipantSheet());
        roomPlayerBox=new FrameLayout(this);roomPlayerBox.setBackground(round(Color.BLACK,LINE,12));View pv=getLayoutInflater().inflate(R.layout.view_player,roomPlayerBox,false);roomPlayerBox.addView(pv,new FrameLayout.LayoutParams(-1,-1));playerView=(PlayerView)pv;adjustedSubtitleView=new SubtitleView(this);adjustedSubtitleView.setVisibility(ccEnabled?View.VISIBLE:View.GONE);adjustedSubtitleView.setClickable(false);adjustedSubtitleView.setFocusable(false);roomPlayerBox.addView(adjustedSubtitleView,new FrameLayout.LayoutParams(-1,-1));playerEmptyOverlay=buildEmptyPlayerOverlay();setPlayerEmptyVisible(false);add(roomContent,roomPlayerBox,-1,dp(235),0,8);
    ...
    private void showParticipantSheet(){
        ...
        for(Map.Entry<String,String> e:participants.entrySet()){
            ...
            String shown=name+(key.equals(localPresenceKey)?"  (You)":"");
    ...
    private void releaseWebViewForMediaSwitch(){
        ...
        if(webViewFullscreen)exitWebViewFullscreen();
        if(webView!=null){
            try{
    ...
    private class WebBridge{
        ...
        @JavascriptInterface public void fullscreen(){ runOnUiThread(()->toggleWebViewFullscreen()); }
    ...
    private void applyMediaSource(JSONObject d){
        String targetKey=d.optString("targetKey","");
        if(!targetKey.isEmpty() && !targetKey.equals(localPresenceKey)) return;
    ...
    private void updatePresenceState(JSONObject state){
        ...
        participants.clear();participantPfps.clear();participantStatuses.clear();typingUsers.clear();
        updateTypingLabel();
        if(localPresenceKey.isEmpty()){participants.put("self",username);participantPfps.put("self",profilePicture);participantStatuses.put("self",localStatus);}
        else {participants.put(localPresenceKey,username);participantPfps.put(localPresenceKey,profilePicture);participantStatuses.put(localPresenceKey,localStatus);}
        try{Iterator<String> it=state.keys();while(it.hasNext()){String key=it.next();JSONObject entry=state.optJSONObject(key);if(entry==null||key.equals(localPresenceKey))continue;JSONArray metas=entry.optJSONArray("metas");String name=key,pfp="",status="Online";if(metas!=null&&metas.length()>0){JSONObject pm=metas.optJSONObject(0);if(pm!=null){name=pm.optString("username",key);pfp=pm.optString("pfp","");status=pm.optString("status","Online");}}participants.put(key,name);if(!pfp.isEmpty())participantPfps.put(key,pfp);participantStatuses.put(key,status);}}catch(Exception ignored){}
        refreshParticipants();
    }
    private void updatePresenceDiff(JSONObject diff){
        try{
            JSONObject joins=diff.optJSONObject("joins");
            if(joins!=null){Iterator<String> it=joins.keys();while(it.hasNext()){String key=it.next();JSONObject entry=joins.optJSONObject(key);String name=key,pfp="";JSONArray metas=entry==null?null:entry.optJSONArray("metas");if(metas!=null&&metas.length()>0){JSONObject m=metas.optJSONObject(0);if(m!=null){name=m.optString("username",key);pfp=m.optString("pfp","");}}boolean wasPresent=participants.containsKey(key);participants.put(key,name);if(!pfp.isEmpty())participantPfps.put(key,pfp);participantStatuses.put(key,entry==null?"Online":(metas!=null&&metas.length()>0&&metas.optJSONObject(0)!=null?metas.optJSONObject(0).optString("status","Online"):"Online"));if(!wasPresent&&shouldShowPresenceNotice(key,true))showPresenceNotice(name+" joined the room",true);}}
            JSONObject leaves=diff.optJSONObject("leaves");
            if(leaves!=null){Iterator<String> it=leaves.keys();while(it.hasNext()){String key=it.next();String name=participants.get(key);boolean existed=participants.remove(key)!=null;participantPfps.remove(key);participantStatuses.remove(key);typingUsers.remove(key);if(name==null)name="Someone";if(existed&&shouldShowPresenceNotice(key,false))showPresenceNotice(name+" left the room",false);}}
        }catch(Exception ignored){}
        runOnUiThread(MainActivity.this::refreshParticipants);
    }
    private void handleRoomPresenceJoin(JSONObject d){
        String key=d.optString("key","");String name=d.optString("username","Someone");String pfp=d.optString("pfp","");String status=d.optString("status","Online");
        if(key.isEmpty()||key.equals(localPresenceKey))return;
        boolean wasPresent=participants.containsKey(key);
        participants.put(key,name);if(!pfp.isEmpty())participantPfps.put(key,pfp);participantStatuses.put(key,status);
        refreshParticipants();
    ...
    private void handleRoomPresenceUpdate(JSONObject d){
        ...
        participants.put(key,name);if(!pfp.isEmpty())participantPfps.put(key,pfp);participantStatuses.put(key,d.optString("status",participantStatuses.getOrDefault(key,"Online")));refreshParticipants();
    }
    ...
    private void handleRoomPresenceLeave(JSONObject d){
        ...
        boolean existed=participants.remove(key)!=null;participantPfps.remove(key);participantStatuses.remove(key);typingUsers.remove(key);updateTypingLabel();refreshParticipants();
    ...
    private void refreshParticipants(){
        ...
        if(roomWatchingLabel!=null)roomWatchingLabel.setText(formatPeopleWatching(count));
    ...
    private void updateTypingLabel(){
        ...
        for(String key:typingUsers){
            String name=participants.get(key);
            if(name==null||name.isEmpty())name="Someone";
            if(!names.contains(name))names.add(name);
        }
        String who=joinStrings(names, ", ");typingLabel.setText(who+(names.size()==1?" is typing…":" are typing…"));typingLabel.setVisibility(View.VISIBLE);
    }
    ...
    private void sendChat(){
        ...
        addChat(username,s,profilePicture,System.currentTimeMillis(),messageId,localPresenceKey);playChatSound();chatInput.setText("");
    }
    ...
    private void addChat(String user,String msg,String pfp,long sentAt,String id){addChat(user,msg,pfp,sentAt,id,"");}
    private void addChat(String user,String msg,String pfp,long sentAt,String id,String senderId){
        runOnUiThread(()->{chats.add(new ChatMessage(user,msg,pfp,sentAt,id,senderId));if(chatList!=null){chatList.getAdapter().notifyItemInserted(chats.size()-1);chatList.scrollToPosition(chats.size()-1);}});
    }
    private void handleTyping(JSONObject d){
        String senderId=d.optString("senderId","");
        String user=d.optString("user","");
        String key=!senderId.isEmpty()?senderId:user;
        if(key.isEmpty()||key.equals(localPresenceKey))return;
        if(senderId.isEmpty() && user.equals(username))return;
        boolean active=d.optBoolean("typing",false);if(active)typingUsers.add(key);else typingUsers.remove(key);
        runOnUiThread(()->updateTypingLabel());
    }
    private void toggleWebViewFullscreen(){
        if(webViewFullscreen) exitWebViewFullscreen(); else enterWebViewFullscreen();
    }
    private void enterWebViewFullscreen(){
        if(webView==null || webViewFullscreen || youtubeFullscreen)return;
        ViewParent parent=webView.getParent();
        if(!(parent instanceof ViewGroup))return;
        webViewOriginalParent=(ViewGroup)parent;
        webViewOriginalIndex=webViewOriginalParent.indexOfChild(webView);
        webViewOriginalLayoutParams=webView.getLayoutParams();
        webViewOriginalParent.removeView(webView);
        webViewFullscreenContainer=new FrameLayout(this);
        webViewFullscreenContainer.setBackgroundColor(Color.BLACK);
        webViewFullscreenContainer.setKeepScreenOn(true);
        webViewFullscreenContainer.addView(webView,new FrameLayout.LayoutParams(-1,-1,Gravity.CENTER));
        ((ViewGroup)getWindow().getDecorView()).addView(webViewFullscreenContainer,new ViewGroup.LayoutParams(-1,-1));
    ...
    private void exitWebViewFullscreen(){
        if(!webViewFullscreen)return;
        View view=webView;
        if(view!=null){ViewParent p=view.getParent();if(p instanceof ViewGroup)((ViewGroup)p).removeView(view);}
        if(webViewFullscreenContainer!=null){ViewParent cp=webViewFullscreenContainer.getParent();if(cp instanceof ViewGroup)((ViewGroup)cp).removeView(webViewFullscreenContainer);}
        if(view!=null && webViewOriginalParent!=null){
            try{
                int index=webViewOriginalIndex;
                if(index<0 || index>webViewOriginalParent.getChildCount())index=webViewOriginalParent.getChildCount();
                if(webViewOriginalLayoutParams!=null)webViewOriginalParent.addView(view,index,webViewOriginalLayoutParams);else webViewOriginalParent.addView(view,index);
            }catch(Exception ignored){}
        }
        webViewFullscreenContainer=null;webViewOriginalParent=null;webViewOriginalLayoutParams=null;webViewOriginalIndex=-1;webViewFullscreen=false;
        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(BG);updateSystemBars();
    }
    ...
    private void disconnect(){
        ...
        typingUsers.clear();participantStatuses.clear();participants.clear();participantPfps.clear();roomWatchingLabel=null;
    ...
    @Override public void onBackPressed(){if(youtubeFullscreen){exitYouTubeFullscreen();return;}if(webViewFullscreen){exitWebViewFullscreen();return;}if(chatOpen){closeChat();return;}if(fullscreen){setFullscreen(false);return;}if(player!=null){disconnect();return;}super.onBackPressed();}
    ...
    @Override protected void onDestroy(){if(pendingSubtitleRunnable!=null)uiHandler.removeCallbacks(pendingSubtitleRunnable);if(webLoadTimeoutRunnable!=null)uiHandler.removeCallbacks(webLoadTimeoutRunnable);uiHandler.removeCallbacksAndMessages(null);try{unregisterReceiver(receiver);}catch(Exception ignored){}if(youtubeFullscreen)exitYouTubeFullscreen();if(webViewFullscreen)exitWebViewFullscreen();releaseWebViewForMediaSwitch();if(playerView!=null){try{playerView.setPlayer(null);}catch(Exception ignored){}}if(player!=null){try{player.release();}catch(Exception ignored){}player=null;}super.onDestroy();}

    private static class ChatMessage{final String user,text,pfp,id,senderId;final long sentAt;String reaction="";ChatMessage(String u,String t,String p,long a,String i,String s){user=u;text=t;pfp=p;sentAt=a;id=i;senderId=s==null?"":s;}}
    ...
    private class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.Holder>{
        ...
        @Override public void onBindViewHolder(Holder h,int pos){
            h.row.removeAllViews();ChatMessage m=chats.get(pos);boolean mine=!m.senderId.isEmpty()&&!localPresenceKey.isEmpty()&&m.senderId.equals(localPresenceKey);
            ...
            if(pos==chats.size()-1&&!typingUsers.isEmpty()){ArrayList<String> names=new ArrayList<>();for(String key:typingUsers){String name=participants.get(key);if(name==null||name.isEmpty())name="Someone";if(!names.contains(name))names.add(name);}String who=joinStrings(names, ", ");TextView typing=text(who+(names.size()==1?" is typing…":" are typing…"),11,MUTED);typing.setPadding(dp(48),dp(2),0,dp(5));h.row.addView(typing,new LinearLayout.LayoutParams(-1,dp(24)));}
        }
