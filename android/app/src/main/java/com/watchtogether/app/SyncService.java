...
public class SyncService extends Service {
    ...
    // Compatibility action names used by the newer MainActivity.
    ...
    private long webSequence = 0;
    private String activeWebSource = "";
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null) {
            ...
            if (ACTION_CONNECT.equals(action)) {
                ...
                try { broadcast("chat", new JSONObject().put("user", username).put("senderId", presenceKey).put("text", intent.getStringExtra("text")).put("pfp", intent.getStringExtra("pfp") == null ? profilePicture : intent.getStringExtra("pfp")).put("sentAt", System.currentTimeMillis()).put("messageId", intent.getStringExtra("messageId") == null ? UUID.randomUUID().toString() : intent.getStringExtra("messageId"))); } catch (Exception ignored) {}
                ...
                try { broadcast("chat_reaction", new JSONObject().put("user", username).put("senderId", presenceKey).put("messageId", intent.getStringExtra("messageId")).put("reaction", intent.getStringExtra("reaction"))); } catch (Exception ignored) {}
                ...
                try {
                    ...
                    d.put("commandSeq", ++webSequence);
                    d.put("eventId", UUID.randomUUID().toString());
                    String requestId = intent.getStringExtra("requestId");
                    if (requestId != null && !requestId.isEmpty()) d.put("requestId", requestId);
                    broadcast("web_sync", d);
                } catch (Exception ignored) {}
            } else if (ACTION_MEDIA_SOURCE.equals(action)) {
                try {
                    String source=intent.getStringExtra("source") == null ? "" : intent.getStringExtra("source");
                    String type=intent.getStringExtra("type") == null ? "local" : intent.getStringExtra("type");
                    if("web".equalsIgnoreCase(type) && !source.trim().isEmpty()) activeWebSource=source.trim(); else activeWebSource="";
                    broadcast("media_source", new JSONObject().put("user", username).put("senderId", presenceKey).put("source", source).put("type", type));
                } catch (Exception ignored) {}
                ...
                try {
                    ...
                    if(forKey!=null&&!forKey.isEmpty()) broadcast("room_presence_ack",new JSONObject().put("forKey",forKey).put("key",presenceKey).put("username",username).put("pfp",profilePicture).put("status", "Online"));
                } catch(Exception ignored){}
                ...
                try {
                    ...
                    presence.put("status", intent.getStringExtra("status") == null ? "Online" : intent.getStringExtra("status"));
    ...
    private void track() {
        ...
        try {
            ...
            broadcast("room_presence_join", new JSONObject().put("key",presenceKey).put("username",username).put("pfp",profilePicture).put("status","Online"));
            if(handler!=null){
                ...
                presenceAnnounceRunnable=()->{
                    ...
                    try{broadcast("room_presence_join",new JSONObject().put("key",presenceKey).put("username",username).put("pfp",profilePicture).put("status","Online"));}catch(Exception ignored){}
    ...
    private void handle(String raw) {
        try {
            ...
            if ("phx_reply".equals(event)) {
                ...
                else if ("media_source".equals(ev)) {
                    String source=data.optString("source","");
                    String type=data.optString("type","local");
                    if("web".equalsIgnoreCase(type) && !source.trim().isEmpty()) activeWebSource=source.trim(); else activeWebSource="";
                    emit("media_source", data);
                }
                else if ("media_source_state".equals(ev)) {
                    String target=data.optString("targetKey","");
                    if(target.isEmpty() || target.equals(presenceKey)){
                        String source=data.optString("source","");
                        activeWebSource=source.trim();
                    }
                    emit("media_source_state", data);
                }
                else if ("chat".equals(ev)) emit("chat", data);
                else if ("chat_reaction".equals(ev)) emit("chat_reaction", data);
                else if ("typing".equals(ev)) emit("typing", data);
                else if ("state_request".equals(ev) && !presenceKey.equals(data.optString("user"))) {
                    String requester=data.optString("user","");
                    String requestId=data.optString("requestId","");
                    emit("state_request", data);
                    if(!requester.isEmpty() && !activeWebSource.isEmpty()){
                        try{
                            broadcast("media_source_state", new JSONObject()
                                    .put("user", username)
                                    .put("senderId", presenceKey)
                                    .put("source", activeWebSource)
                                    .put("type", "web")
                                    .put("targetKey", requester)
                                    .put("requestId", requestId));
                        }catch(Exception ignored){}
                    }
                }
                else if ("room_presence_join".equals(ev) && !presenceKey.equals(data.optString("key"))) { emit("room_presence_join", data); try{broadcast("room_presence_ack",new JSONObject().put("forKey",data.optString("key","")).put("key",presenceKey).put("username",username).put("pfp",profilePicture).put("status","Online"));}catch(Exception ignored){} }
