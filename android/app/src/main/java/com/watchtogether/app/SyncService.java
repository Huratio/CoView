...
public class SyncService extends Service {
    ...
    private String mediaSource = "", mediaSourceType = "local";
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null) {
            ...
            if (ACTION_CONNECT.equals(action)) {
                clientSlot = intent.getIntExtra("client_slot", clientSlot);
                ...
                try { broadcast("typing", new JSONObject().put("user", username).put("typing", intent.getBooleanExtra("typing", false))); } catch (Exception ignored) {}
                ...
                try {
                    ...
                    presenceStatus=playing?"Playing":"Paused";
                ...
                try {
                    ...
                    d.put("position",intent.getLongExtra("position",0)); d.put("playing",playing);
                    ...
                    d.put("sentAt",sentAt); d.put("seq",++syncSequence); d.put("eventId",UUID.randomUUID().toString());
                ...
                mediaSource = intent.getStringExtra("source");
                if (mediaSource == null) mediaSource = "";
                mediaSourceType = intent.getStringExtra("type");
                if (mediaSourceType == null || mediaSourceType.isEmpty()) mediaSourceType = "local";
                try { broadcast("media_source", new JSONObject().put("source", mediaSource).put("type", mediaSourceType).put("user", presenceKey)); } catch (Exception ignored) {}
            } else if (ACTION_STATE_REQUEST.equals(action)) {
                try {
                    String requestId = intent.getStringExtra("requestId");
                    if (requestId == null || requestId.isEmpty()) requestId = UUID.randomUUID().toString();
                    if (!mediaSource.isEmpty()) broadcast("media_source_state", new JSONObject().put("source", mediaSource).put("type", mediaSourceType).put("requestId", requestId).put("user", presenceKey));
                ...
                try {
                    ...
                    presence.put("status", presenceStatus);
        ...
        try {
            ...
            if ("phx_reply".equals(event)) {
                ...
                if ("sync".equals(ev)) emit("sync", data);
                    ...
                    String src=data.optString("source","").trim();
                    if(!src.isEmpty()){ mediaSource=src; mediaSourceType=data.optString("type","web"); }
                    ...
                    try{broadcast("room_presence_ack",new JSONObject().put("forKey",data.optString("key","")).put("key",presenceKey).put("username",username).put("pfp",profilePicture));}catch(Exception ignored){}
                    if(!mediaSource.isEmpty()) try{broadcast("media_source_state",new JSONObject().put("source",mediaSource).put("type",mediaSourceType).put("user",presenceKey));}catch(Exception ignored){}
                }
    ...
    private void emit(String event, JSONObject data) {
        ...
        i.putExtra("client_slot", clientSlot);
